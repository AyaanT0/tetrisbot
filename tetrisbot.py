...
class Game:
    ...
    async def end_game(self):
        ...
        embed.set_footer(text="made by ayaan because ian sucks", icon_url="https://i.imgur.com/n2SQXm7.png")
        await self.message.edit(embed=embed)
...
async def on_ready():
    ...
    print("tetris bot is ALIVE! good job for not screwing up")
...
async def on_message(message):
    ...
    if message.content.lower().startswith(">support") or message.content.lower().startswith(">contact"):
        await message.channel.send("if you actually need help playing tetris, you are worse than i am. and thats really saying something.")
    ...
    if message.content.lower().startswith(">pause"):
        if message.author not in games or games[message.author].game_over:
            await message.channel.send("you're already not playing tetris. you cant pause anything")
